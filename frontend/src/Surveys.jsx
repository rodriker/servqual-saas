...
export default function Surveys({ dims }) {
  ...
  const addPregunta = () => setPreguntas([...preguntas, { dimension: '', texto: '' }]);
  ...
  const handleCreateFull = () => {
    ...
    fetch('http://127.0.0.1:8000/api/v1/surveys-full/', {
      ...
      .then(() => fetch('http://127.0.0.1:8000/api/v1/surveys/')
        .then(r => r.json())
        .then(setSurveys))
      .then(() => {
        setNewTitle('');
